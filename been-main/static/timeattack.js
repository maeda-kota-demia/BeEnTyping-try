document.addEventListener('DOMContentLoaded',function() {
    ...
    var wordObjList = [];
    // var word = "";
    fetch('word-hi-3.csv').then(response => response.text()).then(data => wordObjListMake(data))
    ...
    window.addEventListener("keydown", (event) => {
        ...
        if(startFlag == 0 && event.key == " "){
            ...
            fetch('word-'+mode+'.csv').then(response => response.text()).then(data => wordObjListMake(data))
